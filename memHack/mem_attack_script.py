...
class solution:
    ...
    def getPassword(self):
        password = ""
        for protected in range(1, 1024):
            mem_ctl_exe_result = self.setProtectMem(protected,1023)
            for i in range (33,127):
                pwd_checker_exe_result = self.checkPassword(password+chr(i))
                if ("SEG" in pwd_checker_exe_result):
                    password += chr(i)
                    break
                if ("Correct" in pwd_checker_exe_result):
                    password += chr(i)
                    print(password)
                    self.password = password
                    return self.password
                
        # I first iteratively block out what parts of memory are accessible from 1 to 1024,
        # and then iterate through every valid character and if it's a seg fault i know I got the 
        # specific character right and can move on to the next character, I return then once I call
        # check password and get back correct
